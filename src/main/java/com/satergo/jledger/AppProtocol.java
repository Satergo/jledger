...
public abstract class AppProtocol {
	 ...
	 */
	protected final <EX extends RuntimeException>void voidCheckError(int expectedSW, Function<Integer, EX> exceptionFactory) throws EX {
		APDUResponse response = device.readAPDU(0);
		if (response.getSW() != expectedSW) {
			throw exceptionFactory.apply(response.getSW());
		}
	}
	 ...
	 */
	protected final <EX extends RuntimeException>APDUResponse checkError(int expectedSW, APDUResponse response, Function<Integer, EX> exceptionFactory) throws EX {
		if (response.getSW() != expectedSW) {
			throw exceptionFactory.apply(response.getSW());
		}
		return response;
